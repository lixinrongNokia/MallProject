...
public class ScrollerNumberPicker extends View {
     ...
     * 选择的内容
     */
    private final ArrayList<ItemObject> itemList = new ArrayList<>();
    /**
     * 设置数据
     */
        ...
        if (move > 0) {
            for (ItemObject itemObject : itemList) {
                if (itemObject.isSelected()) {
                    newMove = (int) itemObject.moveToSelected();
                    if (onSelectListener != null)
                        onSelectListener.endSelect(itemObject.id,
                                itemObject.itemText);
                    break;
                }
            }
    ...
    private class ItemObject {
         ...
         */
        boolean isSelected() {
            return (y + move) >= controlHeight / 2 - unitHeight / 2 + 2 && (y + move) <= controlHeight / 2 + unitHeight / 2 - 2 || (y + move + unitHeight) >= controlHeight / 2 - unitHeight / 2 + 2 && (y + move + unitHeight) <= controlHeight / 2 + unitHeight / 2 - 2 || ((y + move) <= controlHeight / 2 - unitHeight / 2 + 2 && (y + move + unitHeight) >= controlHeight / 2 + unitHeight / 2 - 2);
        }
